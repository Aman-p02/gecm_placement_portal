...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_activity') {
    ...
    if (empty($title) || empty($description)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO placement_activities (title, description, activity_year) VALUES (?, ?, ?)");
            $stmt->execute([$title, $description, $activityYear]);
...
$stmt = $pdo->query("SELECT * FROM placement_activities ORDER BY activity_year DESC, created_at DESC");
